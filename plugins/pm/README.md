...
`/pm:auto` wraps the execute → verify pair in an autonomous loop — each phase runs in an isolated subagent with a fresh context, state lives in the task files — until no ready tasks remain, then runs a final full-suite gate (`/pm:gate`) before declaring the work release-ready. You still run `/pm:complete` per task and `/pm:release` at the end.
...
The loop: verify any `done-pending-verify` backlog first, then execute the lowest-id ready task → verify it. ACCEPT → next ready task. REJECT → re-execute the same task. When every task is done it runs a **final full-suite gate** (`/pm:gate`) — the whole test suite plus a holistic re-check of the goals and all acceptance criteria — to catch the cross-cutting regressions per-task verification can't see; on gate failure it auto-remediates and re-gates up to `--max-retries` times. It only suggests `/pm:release` once the gate passes. It stops and hands back to you on: all tasks done and **gate passed** (suggests `/pm:release`), all tasks done but the **gate failed** after remediation (prints the gate report, no release), the same task rejected `--max-retries` times (default 2 — prints the verifier notes verbatim), an executor blocker (prints the `## Blocker` verbatim), or no eligible tasks left (e.g. everything remaining is claimed by teammates). A final summary lists completed, rejection-capped, skipped-claimed, blocked tasks, and the final-gate result.

**Parallel mode (`--parallel [N]`, opt-in; sequential is the default).** Runs up to N independent task pipelines (those whose `depends_on` are all done) concurrently. To do this safely it gives each task its **own git worktree** and the execute worker **commits to a per-task branch** so the independent verifier can review it cold; on ACCEPT the work is **merged back into your current branch** (serialized) so dependents see it. Merge conflicts between concurrently-completed tasks are auto-resolved, then the merged result is re-verified; an unresolvable merge or a failed re-verify leaves that task's branch for you to integrate by hand. Because this **commits to your current branch**, run it on a feature branch, not `main` (it warns if you don't). The worktrees are built from committed state, so it requires a **clean working tree** — it refuses to start on a dirty tree (and offers to commit your `.pm/` task files first), since uncommitted files are invisible to the isolated workers. A per-task failure doesn't halt the run — in-flight work drains and the summary reports every outcome (completed, blocked, rejection-capped, anomalies, merge-failed, unreachable). Parallel mode requires the Workflow tool; without it the flag degrades to the sequential loop.

What auto does NOT do: it never runs `/pm:claim`, `/pm:complete`, or `/pm:release`, never passes `--force`, never pushes, and skips tasks claimed by someone else rather than taking them over. The final gate is read-only on code (it runs the suite and writes only `gate-report.md`); on gate failure a remediation worker fixes the cross-cutting issues — in **sequential** (default) mode those edits, like all other work, stay uncommitted; **`--parallel` mode** is the one exception — it commits accepted work and any gate remediation to per-task branches and to your current branch (but still never pushes or opens PRs). In multi-dev settings, `/pm:claim` before `/pm:auto` is still the recommended courtesy.
...
#### `/pm:gate [slug] [version]`
A Release QA / Integration Lead persona that runs the **final full-suite gate** on a version where every task is `done`. It runs the project's full test suite (the `testing.md` §5 CI-gating command when present, else the detected suite) and holistically cross-checks the *cumulative* implementation against the PRD goals and every task's acceptance criteria — hunting the cross-cutting regressions single-task `/pm:verify` can't catch (shared locale/config/schema, global middleware, end-to-end flows). It is read-only on code: it writes only `.pm/<slug>/<version>/gate-report.md` and returns PASS/FAIL with specific, actionable failures. `/pm:auto` dispatches it automatically when all tasks finish (and auto-remediates on failure), and `/pm:release` runs it as a hard pre-release gate. You can also run it standalone as an "am I actually ready?" check.

```
/pm:gate recurring-s3-exports            # gate the active version
/pm:gate recurring-s3-exports v1         # gate a specific version
```

A missing test suite is a soft warning, not a hard fail (the holistic goal/criteria check still runs); red tests or unmet goals/criteria are a FAIL.

#### `/pm:release <slug> [version]`
Closes out a version. Refuses to release if any task isn't `done`, **and runs `/pm:gate` as a final full-suite gate — refusing to release if the gate fails** (printing the failures so you can fix them, or run `/pm:auto` which re-gates and auto-remediates). Asks for the release tag, deviations from goals, evidence links (PRs/SHAs), and known limitations carrying forward. Writes a frozen `RELEASE.md` and optionally maintains a `CHANGELOG.md` at the project root.
...
| Tier | Commands | Why |
|---|---|---|
| **Opus** (judgment / synthesis) | `/pm:prd`, `/pm:express`, `/pm:amend`, `/pm:research`, `/pm:rerun-research`, `/pm:architect`, `/pm:test`, `/pm:plan`, `/pm:replan`, `/pm:verify`, `/pm:gate`, `/pm:handoff`, `/pm:version` | Multi-source synthesis, two-persona interviews, architecture and task decomposition with dependencies, independent acceptance-criteria judgment, distilling session context into a useful handoff. Express runs PRD interview, persona selection, and task decomposition in one pass — compression is high-judgment work. |
...
**`/pm:auto` preserves the tiering across its loop:** the orchestrator stays on Sonnet (task picking, status checks — mechanical), while each execute subagent runs on Sonnet and each verify subagent on Opus, matching what `/pm:execute` and `/pm:verify` would use standalone. The final-gate subagent runs on Opus (matching `/pm:gate`); its remediation worker runs on Sonnet (code).
...
Run `/pm:claim <slug>` (or `/pm:claim <slug> <id>` for a specific task). It pulls latest, creates the `pm/<slug>/<id>-<slug>` branch, flips status to `in-progress`, sets `assignee` from your git config, commits, and pushes — all in one step. The claim lives on that branch (it's never merged to `main`), so `/pm:next`, `/pm:status`, `/pm:claim`, and `/pm:execute` **discover open claims by scanning the `pm/<slug>/*` branches on the remote** — teammates immediately see the task is taken and won't be handed it as "next ready". If two people race, the second `/pm:claim` refuses up front ("already claimed by …"); the branch-push collision is the final backstop. Optimistic locking via git, no central server.
